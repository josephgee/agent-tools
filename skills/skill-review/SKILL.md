...
If an agent loads this skill and follows it, will it actually accomplish the task the skill promises?
**Evaluate:**
...
- **Phantom prohibitions.** Text telling the agent not to do something no reader was headed toward. Naming a wrong action concretely makes it available: a skill that spends three lines describing what it doesn't want has put that thing in context, and agents reach for whichever branch names the most concrete action. Test each prohibition — would a competent agent, with the rest of the skill in front of it, actually reach for the banned action? If yes, it earns its line; if no, cut or invert it. Typical hits: definitional negatives ("this is not an X" — say what it is; the contrast usually falls out); rationale sections defending a road not taken ("Why we don't use Y" for an approach the skill never proposed); triggers that open with what doesn't fire them before what does; scope disclaimers restating a boundary drawn elsewhere in the skill. Exempt: Rules in Force headers (countering a known failure mode is their whole job); a fallback explicitly ruled out when its gate doesn't hold (a required negation); and any prohibition that passes the test — tracking rollback points by commit sha, weakening a test to make it pass, attesting "nothing to report" are all things an agent genuinely reaches for. Prefer inversion to deletion: "not how a slice is built internally" → "a slice's internal design belongs to the skill executing it"; delete outright only when the positive form says nothing the surrounding text hasn't. Report this as one whole-document finding with a line count, not one finding per instance — it's a density problem, and a per-instance list buries the exempt cases that should survive.
...
- **Rule durability across a long session.** The SKILL.md body is read into the conversation once, so it sits in the compressible region of context — unlike a system prompt or `CLAUDE.md`. Over a long run it is diluted by position (attention is U-shaped, and the middle is the worst place to be) and then lost outright at compaction. Ask: if this skill runs for dozens of turns, do its non-negotiable rules have anywhere to live besides the body? The fix is a short rules-in-force header on the skill's own session artifact — the hard steps only, written verbatim at creation, never edited, re-read at the top of each repeating unit (each cycle, pass, or checkpoint) — which both survives compaction and moves the rules to the end of the context. Flag it as a finding when a long-running skill has an artifact carrying *state* but no durable copy of its *rules*; flag it equally when the header exists but has grown long enough to be its own context cost, or when the skill says to re-read "if unsure" rather than at a fixed point (a judgment call is exactly what decays). A short skill that finishes in a few turns needs none of this — calibrate to session length, not to file size.
...
- **Calibrate to purpose.** Judge fitness for purpose, not adherence to a template.
