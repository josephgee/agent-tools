...
`refactor` or `scaffolding` with a one-sentence reason. Note that MAKE ROOM already absorbs
small preparatory restructuring into the pass that needs it; a `refactor` PR is for what MAKE
ROOM would swamp.
...
is a review that doesn't actually happen — and here, a batch that stops fencing anything.
...
Outside-in starts with an end-to-end test, often over stubbed internals. That is still the right
place to start, but a stubbed flow can be unsafe to merge — it can expose a half-working path to
real users. Choose one of two forms for the first PR and record which in the state file:
...
Note the distinction from RED's **raising skeleton**, which is a within-pass device: raising
stubs exist so the batch fails on assertions rather than imports, and they are replaced during
GREEN of that same pass. An inert *first PR* is a shipping decision about reachability. A PR
whose raising stubs are still reachable at SHIP is not shippable — make it inert or make it
real.
...
1. **Export endpoint returns an empty CSV** — route exists, correct content type and headers, no
   ...
   *Batch: endpoint responds 200; content-type is text/csv; body is the header row only.*
...
The PR plan evolves the same way the design hypothesis does. What you learn in a pass can reveal
