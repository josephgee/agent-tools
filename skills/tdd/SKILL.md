...
These three are deliberately narrow — the smells that most often show up within a single cycle's diff. If you sense pressure but none of the three fits, or want the precise term for a backlog entry, invoke the `design-principles` skill and consult its `design-catalog.md` for the precise name (Fowler's smells, Martin's *Clean Code* heuristics, component-level smells, Martin's symptoms of rot); if that skill is not available, the three triggers above are your floor for the mid-cycle check. Either way this is a quick naming lookup, not a scan — the full catalog belongs to the final review pass below, not the one-minute check.
...
4. **Code is clean**: conduct a dedicated final review of the full codebase — duplication, naming, structure, dead weight. This is a separate end-of-feature pass, distinct from both per-cycle REFACTOR and the per-PR review at SHIP. Cycles clean up locally, SHIP cleans up one increment, and this pass looks at the whole — including seams between PRs, where duplication most easily survives, since no single PR's diff contains both sides of it. Systematically check the codebase by invoking the `design-principles` skill and working through its `design-catalog.md` (and `test-catalog.md`) at every altitude — the per-cycle check only screens for three fast triggers, so this is where the fuller catalog gets applied, including the component-level entries that no single cycle's diff can show. If that skill is not available, fall back to the refactor-checklist and the three per-cycle triggers as your floor. (To hand this pass to a separate reviewer instead, invoke the `design-review` skill with an explicit scope — the feature branch against its base — so it reviews without stopping to ask; it runs exactly this diagnosis and reports a rated assessment.) New findings go to the backlog and must be resolved before declaring done; if a finding is substantial, it becomes another PR in the plan rather than an amendment to one already shipped.
