...
These three are deliberately narrow — the smells that most often show up within a single cycle's diff. If you sense pressure but none of the three fits, or want the precise term for a backlog entry, invoke the `design-principles` skill and consult its `design-catalog.md` for the precise name (Fowler's smells, Martin's symptoms of rot); if that skill is not available, the three triggers above are your floor for the mid-cycle check. Either way this is a quick naming lookup, not a scan — the full catalog belongs to the final review pass below, not the one-minute check.
