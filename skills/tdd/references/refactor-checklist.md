...
- **Comments**: delete every comment in the diff. Where does the code need to be more expressive to make up for it? Do that. Decision context (why this change, what was rejected) goes in the PR description, not back into the code — hold it in this cycle's `Learned` line in the state file until SHIP.
